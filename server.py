...
# --- IMPROVEMENT 1: Larger Filter Window ---
# Increased from 5 to 15 to smooth out random signal spikes
FILTER_WINDOW_SIZE = 15 

# --- IMPROVEMENT 2: Stabilization Buffer ---
# We must see the same block this many times before switching the display.
...
class AnchorFilter:
    ...
    def update(self, anchor_id, rssi):
        ...
        if anchor_id not in self.history:
            # CHANGE: Using larger window size
...
class ZoneStabilizer:
    ...
    def update(self, new_zone):
        ...
        # LOGIC: Only switch display if the new zone is the majority
        # in our history buffer.
        if len(self.history) == STABILIZATION_COUNT:
            # Count occurrences of each zone in buffer
            counts = Counter(self.history)
            most_common, count = counts.most_common(1)[0]
            
            # If > 50% of recent calculations agree, switch.
...
def print_dashboard():
    ...
    for mac, info in sorted_devs:
        ...
        # 1. Calculate the Instant Zone (Noisy)
        calculated_zone = identify_block_logic(l, c, r)
        
        # 2. Feed into Stabilizer to get Smooth Zone
    ...
    try:
        ...
        if mac and mac.lower() == TARGET_MAC.lower():
            if mac not in active_devices:
                # Initialize both Filter and Stabilizer for new device
                active_devices[mac] = {
                    "filter": AnchorFilter(),
